...
class GabaritManager extends manager
{
     ...
     *
     * @return \Solire\Lib\Model\gabarit
     */
    public function getGabarit($id_gabarit)
    {
        ...
        $this->gabarits[$id_gabarit] = $gabarit;
        return $this->gabarits[$id_gabarit];
    }
     ...
     * @param bool   $main       ???
     *
     * @return \Solire\Lib\Model\GabaritPage|array tableau de page
     */
    ...
    protected function saveMeta($page, $donnees)
    {
        ...
        if ($updating) {
            ...
            $rewriting = $this->db->rewrit($titre_rew, 'gab_page', 'rewriting', $query);
    ...
    public function saveBloc(
        ...
        if (
            count($bloc->getGabarit()->getJoins()) == 1
    ...
    protected function previsuBloc($bloc, &$donnees)
    {
        ...
        foreach ($donnees['id_' . $gabarit->getTable()] as $id_bloc) {
            ...
            foreach ($champs as $champ) {
                ...
                if (
                $champ['type'] != 'WYSIWYG' && $champ['type'] != 'TEXTAREA'
                ) {
                    $value = str_replace('"', '&quot;', $value);
                }
